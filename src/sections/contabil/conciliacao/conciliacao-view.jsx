...
import { Box, Card, Stack, Alert, Button, Select, MenuItem, Typography, CardContent } from '@mui/material';
...
export default function ConciliacaoPageView({ cliente, extratos, currentDate }) {
  ...
  const processFiles = async (files) => {
    try {
      ...
      for (const file of files) {
        ...
        const bankId = parsedData.OFX.BANKMSGSRSV1?.STMTTRNRS?.STMTRS?.BANKACCTFROM?.BANKID || 'Desconhecido';
        const accountId = parsedData.OFX.BANKMSGSRSV1?.STMTTRNRS?.STMTRS?.BANKACCTFROM?.ACCTID || 'Conta Desconhecida';
        ...
        bancosData[bankId].lancamentos = [...bancosData[bankId].lancamentos, ...formattedTransactions];
      }
    ...
    { field: 'tipo', headerName: 'Tipo', flex: 0.5 },
    {
      ...
      renderCell: (params) => (
        <Select
          ...
          onChange={(e) => handleConciliacaoChange(params.row.id, params.row.bancoId, e.target.value)}
          fullWidth
        >
  ...
  const renderExtratos = () => Object.entries(bancos).map(([bankId, { lancamentos, periodoInicio, periodoFim }]) => (
      <Card key={bankId} sx={{ mb: 3 }}>
        <CardContent>
          <Typography variant="h6">Banco: {getBancoNome(bankId)}</Typography>
          <Typography variant="subtitle2">
            Período: {formatarData(dayjs(periodoInicio).format('YYYYMMDD'))} - {formatarData(dayjs(periodoFim).format('YYYYMMDD'))}
          </Typography>
          <Typography variant="subtitle2">Status: {status}</Typography>
          <DataGrid rows={lancamentos} columns={columns} autoHeight />
          <Stack direction="row" justifyContent="flex-end" spacing={2} mt={2}>
            <Button variant="contained" color="primary" onClick={() => salvarExtratoConciliado(bankId)}>
              Salvar Extrato
            </Button>
            <Button variant="contained" color="secondary" onClick={() => finalizarConciliacao(bankId)}>
